...
function Pin({ data, onBack, onOk }) {
  ...
  const go = () => {
    ...
    if (w) onOk(w);
    else { setErr('PIN incorrecte'); setPin(''); }
  };
  return (
    <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
...
function Worker({ user, data, save, reload, onOut }) {
  ...
  const addRest = async () => {
    ...
    if (form.shift === 'both') {
      if (!form.h1 || !form.h2 || !form.h3 || !form.h4) return alert('Omple totes les hores');
    } else {
      if (!form.h1 || !form.h2) return alert('Omple les hores');
    }
    setSaving(true);
    ...
    const entry = {
      id: Date.now() + '', od_id: user.id, name: user.name, date: form.date, loc_id: form.locId, loc_name: loc?.name || '',
      type: 'restaurant', shift: form.shift, job: form.job, hora_in: form.h1, hora_out: form.h2,
      hora_in2: form.h3 || null, hora_out2: form.h4 || null, horari: horari, hours: hrs,
      rate: user.rate, total: form.shift === 'extra' ? hrs * user.rate : p[form.shift], note: form.note
    };
  ...
  const addCat = async () => {
    ...
    const entry = {
      id: Date.now() + '', od_id: user.id, name: user.name, date: form.date, loc_id: form.locId, loc_name: loc?.name || '',
      type: 'catering', job: form.job, hora_in: form.h1, hora_out: form.h2, hours: hrs,
      rate: user.rate, car: form.car, km, km_cost: km * 0.26, total: hrs * user.rate + km * 0.26, note: form.note
    };
    await supabase.from('entries').insert([entry]);
    await reload();
    setForm({ date: '', locId: '', shift: '', job: '', h1: '', h2: '', h3: '', h4: '', note: '', car: false, km: '' });
...
function Admin({ data, save, reload, onOut }) {
  ...
  const ents = data.entries.filter(e => {
    ...
    return period === 'setmana' ? d >= ws && d <= we : d >= ms && d <= me;
  });
  ...
  const addW = async () => {
    ...
    const newWorker = { id: Date.now() + '', name: nw.n.toUpperCase(), surname1: nw.s1.toUpperCase(), surname2: nw.s2 ? nw.s2.toUpperCase() : '', pin, rate: +nw.r };
  ...
  const toggleLoc = async (id, active) => {
        ...
        {tab === 'treballadors' && <div className="space-y-3">
          <div className="bg-white rounded-lg shadow p-4 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <input placeholder="2n cognom (opcional)" value={nw.s2} onChange={e => setNw({ ...nw, s2: e.target.value })} className="flex-1 p-3 border rounded" />
            </div>
            <div className="flex gap-2">
              ...
              <div key={w.id} className="p-4">
                {editW === w.id ? (
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <input type="number" value={editWV.r} onChange={e => setEditWV({ ...editWV, r: e.target.value })} className="w-20 p-2 border rounded" placeholder="€/h" />
